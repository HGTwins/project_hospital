...
export default function JoinPage() {
  ...
  const checkDuplicate = async (type: string) => {
    ...
    try {
      const response = await fetch(`http://140.245.77.74:8080/api/check-duplicate?type=${type}&value=${value}`);
  ...
  const handleJoin = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("http://140.245.77.74:8080/api/join", {
        method: "POST",
