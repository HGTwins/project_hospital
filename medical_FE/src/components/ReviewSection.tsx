...
export default function ReviewSection({ hospitalId }: { hospitalId: number }) {
  ...
  const fetchReviews = useCallback(async (token: string) => {
    try {
      const res = await fetch(`http://140.245.77.74:8080/api/review/hospitalId/${hospitalId}`, {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`http://140.245.77.74:8080/api/review`, {
        method: 'POST',
        headers: {
  ...
  const handleUpdate = async (seq: number) => {
    ...
    const token = sessionStorage.getItem('jwtToken');
    const res = await fetch(`http://140.245.77.74:8080/api/review/${seq}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': token!.startsWith('Bearer ') ? token! : `Bearer ${token}`
