...
- 김현지:
    - 배운점 
        1) 깃허브로 브랜치를 사용하고 병합을 하면서 협업의 흐름을 이해하게 됐음
        2) 긴 코드를 여러 컴포넌트로 분리하면서 구조적으로 설계하는 방법을 배웠음
        3) 문서화없이 구두로 의사소통을 하다보니 코드를 수정하는 일이 잦아 팀 프로젝트를 하면서 문서화가 필요하다는 것을 체감함
    - 한계점
       1) 지도에 마커수가 많아지면서 화면 반응 속도가 느려져서 최적화 하는 방법을 찾아야 할 것 같았음
       2) 컴포넌트 분리가 미흡해서 구조도 복잡해지고, 유지보수도 어려워져서 컴포넌트 역할을 더 명확하게 나누는게 필요해 보임
