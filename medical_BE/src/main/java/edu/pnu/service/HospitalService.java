...
public class HospitalService {
	...
	public Page<HospitalDto> getHospitalInfo(MedicalPageSearch mps) {
		...
		if (mps.getSidoName() == null && mps.getSigunguName() == null) {
			...
			return basicRepo.findBySidoCodeSidoName(mps.getSidoName(), pageable).map(HospitalDto::from);
			// 시도 및 시군구별 병원
		} else {
			return basicRepo
					.findBySidoCodeSidoNameAndSigunguCodeSigunguName(mps.getSidoName(), mps.getSigunguName(), pageable)
	...
	public HospitalDto findById(Long hospitalId) {
		return basicRepo.findById(hospitalId).map(HospitalDto::from)
	...
	public Page<EssentialHospitalDto> getPageEssential(MedicalPageSearch mps, String deptCode) {
		...
		if (deptCode != null) {
			...
			if (mps.getSidoName() == null && mps.getSigunguName() == null) {
				...
				essential = deptRepo.getPageByEssentialAndSidoNameAndSigunguNameAndDeptCode(mps.getSidoName(),
						mps.getSigunguName(), deptCode, pageable);
			}
		} else {
			// 전체
			if (mps.getSidoName() == null && mps.getSigunguName() == null) {
				...
				essential = deptRepo.getPageByEssentialAndSidoNameAndSigunguName(mps.getSidoName(),
						mps.getSigunguName(), pageable);
			}
		}
		...
		return essential.map(hospital -> {
			...
			String deptNames = hospital.getDeptDoctors().stream()
					...
					.map(dd -> dd.getDeptCode().getDeptName()).collect(Collectors.joining(", "));
	...
	public Page<OperationInfoDto> getPageHolidayOpen(MedicalPageSearch mps) {
		...
		if (mps.getSidoName() == null && mps.getSigunguName() == null) {
			...
			return operRepo.getPageByHolidayOpenAndSidoNameAndSigunguName(mps.getSidoName(), mps.getSigunguName(),
					pageable);
		}
	}
	...
	public Page<OperationInfoDto> getPageNightOpen(MedicalPageSearch mps) {
		...
		if (mps.getSidoName() == null && mps.getSigunguName() == null) {
			...
			return operRepo.getPageByNightOpenAndSidoNameAndSigunguName(mps.getSidoName(), mps.getSigunguName(),
					pageable);
		}
	}
